...
3. 아웃라인 위키 발행은 권한 정책(요청되지 않은 외부 발행)으로 건너뜀. 보고서는 `docs/`에 있다.
4. `build.ps1`이 PowerShell 5.1에서 pip stderr를 오류로 취급해 중단됨 — 우회 실행 중, 보완은 백로그.
5. 랜딩 페이지 폰트 번들(라이선스 확인 필요), 단일 인스턴스 가드(M1).
